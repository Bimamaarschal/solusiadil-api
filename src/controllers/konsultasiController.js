...
const createKonsul = async (req, res, next) => {
    try {
      const { id_konsultasi, tanggal, id_masyarakat, nama_mast, id_apph, nama_apph, calendly, judul, pertanyaan, jawaban, keterangan, jenis, undangundang, status, lanjutan1, wilayahhukum, referensi, media } = req.body;
      if ( !id_konsultasi || !tanggal || !id_masyarakat || !nama_mast || !id_apph || !nama_apph || !calendly || !judul || !pertanyaan || !jawaban || !keterangan || !jenis || !undangundang || !status || !lanjutan1 || !wilayahhukum || !referensi || !media ) {
        return res.status(400).json({ message: 'Kesalahan Dalam Menyimpan Data' });
      }
      const konsultasiData = { id_konsultasi, tanggal, id_masyarakat, nama_mast, id_apph, nama_apph, calendly, judul, pertanyaan, jawaban, keterangan, jenis, undangundang, status, lanjutan1, wilayahhukum, referensi, media };
